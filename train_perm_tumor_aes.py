...
def main(training_seed, max_depth, device, model_suffix, num_epochs=10, save_epochs=[]):
  ...
  for depth in trange(0, max_depth+1):
    ...
    for perm_seed in perm_seeds:
      if depth > 0:
        ...
        assert (cell_types_in_order[perm_ids] == cell_types_in_order).all()
      elif perm_seed != perm_seeds[0]:
        continue  # nothing to permute for depth 0, so skip all but first seed 
      else:
        perm_ids = None
      _ = train_triplet(model=None, 
                      model_path=MODEL_PREFIX + MODEL_KIND + '_lr1em4_e{:d}_b128_h1_pd_pre_ts{:d}_perm{:d}d{:d}type_g{:d}{:s}.pt'.format(
                          num_epochs, training_seed, perm_seed, depth, dim, model_suffix), 
                      training_seed=training_seed, n_genes=dim, h=1, gene_matrix=expression, apn_lut_path=LUT_PATH, 
...
if __name__ == "__main__":
  ...
  for training_seed in [112221, 6243321, 99483, 92231, 555242]:
    main(training_seed, args.max_depth, device=args.device, model_suffix=args.model_suffix, 
         num_epochs=args.epochs, save_epochs=args.save_epochs)
